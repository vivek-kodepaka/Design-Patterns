...
```
above code is singleton with thread safe
drawbacks of above approach
  1. eager initialization  -> cause to load objects even if we don't need , causes Startup time becomes slow
  2. Hard to handle dependencies / configuration
      - Suppose you need values like:
          - DB URL from env, Spring configs, dynamic tenant settings
          - But eager singleton is created before you can inject/provide them properly
...
public class Database {
    ...
    public synchronized void init(String url, String user, String pass) {
        if (initialized) {
